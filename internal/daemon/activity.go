...
func (c *conn) activity(req protocol.ControlReq, snap supervisor.Snapshot, lines int) (protocol.LogsData, error) {
	...
	target := runtrace.Scope{Name: req.Name, Adapter: h.TrajectoryKind(), Workdir: supervisor.Workdir(h), Args: h.Args}
...
func (c *conn) peerScopes(target runtrace.Scope) ([]runtrace.Scope, []string) {
	...
	for _, snap := range mgr.Snapshots() {
		...
		p := runtrace.Scope{Name: snap.Name, Adapter: h.TrajectoryKind(), Workdir: supervisor.Workdir(h), Args: h.Args}
		if p.Workdir == "" {
			p.Workdir = daemonDir
		}
