...
//
// @joestump-agent 09/22/2026 - Lease asked every tick on the tick's clock (it
// was a wall-clock read the pass then re-judged, and in hours it was never
// asked, so a lease outlived the window that opened under it); a graceful
// hold steps its close on the same tick.
