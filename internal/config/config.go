...
func Parse(data []byte, filename string) (*core.Config, error) {
	...
	for _, h := range headers {
		...
		// Source Table".
		case len(h.parts) == 1 && h.parts[0] == core.SourceKindChannel:
			continue
		case len(h.parts) == 1 && h.parts[0] == core.SourceKindWebhook:
			continue
...
func registerHarness(cfg *core.Config, filename, name string, line int, rh rawHarness, defaultEnabled bool, resolve func(string) string) error {
	...
	if rh.CatchUp != nil && schedule == "" && !hasChannelTrigger && strings.TrimSpace(rh.OperatingHours) == "" {
		return newError(filename, line,
			"harness %q: \"catch_up\" requires \"schedule\", a channel trigger, or \"operating_hours\" (nothing else can miss a firing: a webhook delivery with nobody listening was refused at the socket)", name)
	}
