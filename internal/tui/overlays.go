...
func buildHarnessForm(fi *formInputs) *huh.Form {
	return huh.NewForm(
		huh.NewGroup(
			...
			huh.NewInput().Title("harness — required (crush/claude-code/codex/pi/omp/generic/command)").Value(&fi.harness),
			...
			huh.NewInput().Title("transcripts (command harness only; claude-code/crush/codex/pi/omp — observe its sessions as that agent's)").Value(&fi.transcripts),
