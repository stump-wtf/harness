...
// and starts the newly-added ones (SPEC-0004 REQ "Project Control Operations").
// It is idempotent in the reconcile sense (SPEC-0004 REQ "Bring Up"): a re-up
// adds new harnesses, stops + deregisters removed ones, and stages changed
// definitions to apply on next restart per SPEC-0003 — a running process is
// never silently bounced, and continuing harnesses are otherwise untouched.
//
...
// is registered.
func (m *Manager) ProjectUp(project string, defs []core.Harness) error {
	...
	m.mu.Lock()
	// The project name must not shadow an existing bare global harness name
	// (SPEC-0004 REQ "Project Naming And Namespacing" scenario "Name collides
	// with a global harness").
	if _, exists := m.cfg.Harnesses[project]; exists {
		m.mu.Unlock()
		return fmt.Errorf("project up %q: %w: a global harness named %q exists",
	...
	for full := range desired {
		if _, taken := m.supervisors[full]; taken {
			if _, ours := rec.harnesses[full]; !ours {
				m.mu.Unlock()
				return fmt.Errorf("project up %q: harness %q: %w: name already registered",
	...
	for full := range rec.harnesses {
		...
		m.order = removeName(m.order, full)
	}
	// Continuing: stage definition changes (SPEC-0003 REQ "Config Change
	// Application" — applied on next restart, a no-op if unchanged).
	// New: register + start.
	...
	// their actor loop processes them).
...
// project afterward (SPEC-0004 REQ "Tear Down"). An unknown project returns an
// error wrapping config.ErrUnknownProject and changes no state.
func (m *Manager) ProjectDown(project string) ([]string, error) {
	...
	for _, s := range sups {
		s.Shutdown() // graceful stop, then the supervisor is gone for good
	}
	m.markDirty()
...
// harness name, checking the global config first and then every project. The
// daemon uses it to project Cmd/Backend/Description into control responses for
// project harnesses exactly like global ones.
...
// atomically). Every failure wraps ErrInvalidProjectDef.
func validateProjectDefs(project string, defs []core.Harness) error {
	...
	for _, h := range defs {
		...
		case strings.TrimSpace(h.Cmd) == "":
			return fmt.Errorf("project up %q: harness %q: %w: missing required cmd",
				project, h.Name, ErrInvalidProjectDef)
