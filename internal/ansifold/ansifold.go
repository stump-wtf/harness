// Package ansifold provides inert-text filtering for raw PTY byte streams.
//
...
// Text makes one line of raw PTY output safe to print inside the cockpit:
...
//
// Eviction case: when the ring evicts the oldest lines, a payload's introducer
// (e.g. the DCS opener \x1bP) may have aged out of the retained tail while the
// continuation bytes remain. The sequential state thread starts fresh at state=0
// on the first retained line, so those continuation bytes are parsed as
// standalone printable characters and emitted as garbage. This is the hard case
// #146 explicitly allowed documenting rather than solving: a purely sequential
// state thread cannot recognize that it is mid-payload without the introducer.
// The ring's retention window (DefaultRingLines = 10,000) makes this path
// unlikely in practice — a payload would need to span more lines than the entire
// retained tail — but it is not impossible.
func Text(line string, initialState byte) (string, byte) {
	var (
		b      strings.Builder
		state  = initialState
		styled bool
...
// Lines applies Text to a slice, threading parser state across line boundaries
// so escape payloads split by ring.Write's newline split are fully suppressed
// (issue #146).
func Lines(lines []string) []string {
	out := make([]string, len(lines))
	var state byte
	for i, ln := range lines {
		out[i], state = Text(ln, state)
	}
	return out
}
