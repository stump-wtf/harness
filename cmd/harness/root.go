...
func newRootCmd() *cobra.Command {
	...
	root.AddCommand(
		...
		newLogsCmd(g),
		newSimpleCmd(g, "jobs", "list triggered harnesses and how their runs went", nameNone),
		newSimpleCmd(g, "triggers", "list trigger sources: state, last event, counters, harnesses", nameNone),
		newTriggerCmd(g),
		newRunsCmd(g),
		newAttachCmd(g),
		newDoctorCmd(g),
