...
related: [ADR-0008, ADR-0013, ADR-0021]
---
...
* **Records written in parallel (cited here, and linked in frontmatter once
  they merge):** ADR-0023 (the `command` kind, whose pins are attestation-only
  through `transcripts`, and the `pi`/`omp` adapters this renders for),
  ADR-0028 (the run ledger that carries `model_mismatch` and the served model),
  ADR-0027 (budgets, which share the observer's per-call usage items),
  ADR-0025 (supervisor-held leases, which fail the todo on a mismatch), and
  ADR-0022 (telemetry export, PR #408).
