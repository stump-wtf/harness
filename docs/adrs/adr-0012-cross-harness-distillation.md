...
> **Not yet implemented.** Distillation and the learned-skill tier described here are design-stage; no distiller or learned-skill store exists in the codebase. Tracked by the SPEC-0007 epic, harness#69.
>
> **Amended by [ADR-0030](adr-0030-grounded-skill-distillation.md)** before implementation: what grounds a skill (merged pull requests, not trajectories alone), how a candidate is verified (blind reconstruction), how it reaches review (pull requests, with merge as promotion), and where skills live (operator-declared skill repos, with the fixed cross-project threshold replaced by each distiller's `min_repos`) are decided there. The search-only tier, FTS5 index and lifecycle rules below still stand.
