...
* **Records written in parallel (cited here, and linked in frontmatter once
  they merge):** ADR-0026, fail-closed model pinning, which renders pins
  through these adapters and attests through `transcripts`. ADR-0025,
  supervisor-held leases and relay attempts, whose one-shots are typically
  `command` or templated harnesses. ADR-0027, budgets, which cannot cap tokens
  on a `command` harness without `transcripts`. ADR-0028, the run ledger, which
  records the `template_unresolved` skip. ADR-0024, the stack installer, whose
  persona templates may use `prompt_template`.
