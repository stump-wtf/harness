...
- **AND** masking SHALL operate one log line at a time, carrying state only
  across the lines of a PEM private key block, so a credential split across
  lines by terminal or TUI wrapping is not recognised
